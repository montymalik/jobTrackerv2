...
const SavedResumesTab: React.FC<ExtendedSavedResumesTabProps> = ({ 
  ...
  const [coverLettersLoading, setCoverLettersLoading] = useState<boolean>(false);
  ...
  const fetchCoverLetters = async () => {
    ...
    try {
      console.log(`Fetching cover letters for jobApplicationId: ${jobId}`);
      ...
      const data = await response.json();
      console.log(`Fetched ${data.length} cover letters for job ${jobId}`);
  ...
  const handleSetCoverLetterAsPrimary = async (coverLetterId: string, event: React.MouseEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to set primary cover letter: ${response.status}`);
      }
      
      // Refresh cover letters after setting primary
      fetchCoverLetters();
      setSuccessMessage("Primary cover letter updated successfully");
    } catch (error) {
  ...
  const handleDeleteCoverLetter = async (coverLetterId: string, event: React.MouseEvent) => {
    ...
    try {
      const response = await fetch(`/api/cover-letter/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: coverLetterId }),
      });
      
      if (!response.ok) {
        throw new Error(`Failed to delete cover letter: ${response.status}`);
      }
      
      // Refresh cover letters after deletion
      fetchCoverLetters();
      ...
      {jobId ? (
          ...
          {activeTab === 'coverLetters' && (
            <>
              {coverLettersLoading ? (
                <div className="flex justify-center items-center py-8">
                  <div className="spinner mr-3"></div>
                  <span className="text-gray-600 dark:text-gray-300">Loading cover letters...</span>
                </div>
                ...
                <div className="text-center py-8 text-gray-600 dark:text-gray-400">
                  ...
                  <p className="text-lg font-medium mb-1">No cover letters found</p>
                  <p>Generate a cover letter in the Cover Letter tab to get started</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {coverLetters.map((coverLetter) => (
                        <tr key={coverLetter.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700 ${coverLetter.id === currentCoverLetterId ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                              {coverLetter.fileName || `Cover Letter ${coverLetter.version}`}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-500 dark:text-gray-400">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-500 dark:text-gray-400">
                              {new Date(coverLetter.createdAt).toLocaleDateString()} {new Date(coverLetter.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            {coverLetter.isPrimary ? (
                              <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200">
                                Primary
                              </span>
                            ) : (
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex justify-end space-x-2">
                              ...
                              {!coverLetter.isPrimary && (
                                <button
                                  onClick={(e) => handleSetCoverLetterAsPrimary(coverLetter.id, e)}
                                  className="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300"
                                  title="Set as Primary"
                                  type="button"
                                >
                              ...
                              <button
                                onClick={(e) => handleDeleteCoverLetter(coverLetter.id, e)}
                                className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300"
        ...
        <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow-sm border dark:border-gray-700">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Please select a job to view associated documents
          </p>
        </div>
      )}
