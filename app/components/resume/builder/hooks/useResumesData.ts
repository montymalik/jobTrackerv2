...
const convertLlmJsonToSections = (jsonData: any): ResumeSection[] => {
  ...
  // Save resume
// Updated handleSave function for useResumeData.ts
// JSON-only approach with no markdown references
...
const handleSave = async () => {
  ...
  try {
    // Get the structured data from sections, with improved HTML preservation
    const structuredData = sectionsToStructuredData(sections);
    
    // Store the complete section data as a backup to ensure no information is lost
    // This helps preserve AI-enhanced content
